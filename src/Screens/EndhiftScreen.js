...
import { View, Text, TouchableOpacity, Dimensions, Keyboard, Modal, TouchableWithoutFeedback, ToastAndroid, FlatList, Alert, ImageBackground, StyleSheet, SafeAreaView, ScrollView, Platform, KeyboardAvoidingView, ActivityIndicator } from 'react-native';
...
const EndShiftScreen = props => {
    ...
    const NextHandler = async () => {
        ...
        if (!editorNumbers) {
            ...
            try {
                ...
                url = userCredentials['end_shift_validation_url']
    ...
    const endShiftHandler = async () => {
        ...
        if (!editorNumbers) {
            ...
            try {
                let finalScaneCodes = scannnedBarCode.join(",")
                url = userCredentials["end_shift_url"]
                scan_codes = editorNumbers === 1 ? finalScaneCodes : editorNumbers,
                    accessToken = userCredentials['accessToken']
                setMissingTicketsData('')
                let response = await fetch(url, {
        ...
        <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
            ...
            <View style={[styles.container, { opacity: modalVisible || waitModalVisible ? 0.5 : 1 }]}>
                ...
                <ScrollView showsVerticalScrollIndicator={false}>
                    ...
                    {nullSubmit ?
                        <Text style={[styles.errorTextNullSubmit, { width: '95%', }]}>Scane Some tickets For End Shift</Text> : <Text></Text>}
